...
Generates hyper-personalized cold and nurture email sequences tailored to ICP segments and buyer intent

## Architecture

```
14-email-personalization-agent/
├── src/
...
```bash
cd agents/14-email-personalization-agent
...
```bash
docker build -t 14-email-personalization-agent .
