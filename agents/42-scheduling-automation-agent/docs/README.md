...
Manages appointment booking, sends reminders, and handles rescheduling to reduce no-show rates

## Architecture

```
42-scheduling-automation-agent/
├── src/
...
```bash
cd agents/42-scheduling-automation-agent
...
```bash
docker build -t 42-scheduling-automation-agent .
