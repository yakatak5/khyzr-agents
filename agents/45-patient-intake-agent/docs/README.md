...
Collects patient demographics, verifies insurance eligibility, and pre-populates EHR fields ahead of visits

## Architecture

```
45-patient-intake-agent/
├── src/
...
```bash
cd agents/45-patient-intake-agent
...
```bash
docker build -t 45-patient-intake-agent .
