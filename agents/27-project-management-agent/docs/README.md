...
Aggregates task updates from PM tools and generates automated status reports for stakeholders

## Architecture

```
27-project-management-agent/
├── src/
...
```bash
cd agents/27-project-management-agent
...
```bash
docker build -t 27-project-management-agent .
