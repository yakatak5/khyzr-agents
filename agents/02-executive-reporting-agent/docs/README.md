...
Pulls data from dashboards and auto-generates narrative board decks with commentary on KPI variances

## Architecture

```
02-executive-reporting-agent/
├── src/
...
```bash
cd agents/02-executive-reporting-agent
...
```bash
docker build -t 02-executive-reporting-agent .
