...
Compiles sustainability metrics and generates ESG reports aligned to GRI, SASB, or TCFD frameworks

## Architecture

```
08-esg-reporting-agent/
├── src/
...
```bash
cd agents/08-esg-reporting-agent
...
```bash
docker build -t 08-esg-reporting-agent .
