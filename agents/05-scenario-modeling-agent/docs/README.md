...
Builds and runs multiple business scenarios dynamically based on variable assumptions from leadership

## Architecture

```
05-scenario-modeling-agent/
├── src/
...
```bash
cd agents/05-scenario-modeling-agent
...
```bash
docker build -t 05-scenario-modeling-agent .
