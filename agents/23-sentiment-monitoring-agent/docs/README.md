...
Aggregates reviews from G2, Trustpilot, and social media to surface sentiment trends and product feedback

## Architecture

```
23-sentiment-monitoring-agent/
├── src/
...
```bash
cd agents/23-sentiment-monitoring-agent
...
```bash
docker build -t 23-sentiment-monitoring-agent .
