...
Monitors stock levels, triggers reorder alerts, and optimizes safety stock based on lead times and demand variability

## Architecture

```
25-inventory-optimization-agent/
├── src/
...
```bash
cd agents/25-inventory-optimization-agent
...
```bash
docker build -t 25-inventory-optimization-agent .
