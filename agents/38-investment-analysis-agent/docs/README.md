...
Models ROI, NPV, and IRR for potential investments using user-defined assumptions and benchmarks

## Architecture

```
38-investment-analysis-agent/
├── src/
...
```bash
cd agents/38-investment-analysis-agent
...
```bash
docker build -t 38-investment-analysis-agent .
