...
Monitors aging AR, generates collection emails, and escalates overdue accounts based on risk tiers

## Architecture

```
40-ar-collections-agent/
├── src/
...
```bash
cd agents/40-ar-collections-agent
...
```bash
docker build -t 40-ar-collections-agent .
