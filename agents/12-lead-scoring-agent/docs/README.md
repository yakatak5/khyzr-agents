...
Ranks inbound and outbound leads by conversion probability using behavioral, firmographic, and intent data

## Architecture

```
12-lead-scoring-agent/
├── src/
...
```bash
cd agents/12-lead-scoring-agent
...
```bash
docker build -t 12-lead-scoring-agent .
